...
(define (merge comp list1 list2)
    (cond
      ((and (null? list1) (null? list2)) nil)
        ((and (null? list1) (list? list2))
          (cons (car list2) (merge comp nil (cdr list2))))
        ((and (null? list2) (list? list1))
          (cons (car list1) (merge comp (cdr list1) nil)))
...
(define (tree-sums tree)
  (define (tree-helper lsts)
    (cond ((null? (cdr lsts)) (car lsts))
      (else (append (car lsts) (tree-helper (cdr lsts))))))
    (cond ((null? (children tree)) (list (entry tree)))
    (else (map (lambda (sum_children) (+ sum_children (entry tree)))(tree-helper (map tree-sums (children tree)))))))
